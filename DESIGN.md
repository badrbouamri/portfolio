...
- **Organisation logos — original colors, no card, tried a legibility compromise, reverted on request.** The owner asked for original colors with no white background. Sampling actual pixel values from all four PNGs found two that can't survive that literally: Stellantis' wordmark is entirely navy-blue (~2:1 contrast on black, fails even large-text 3:1) and ENSET's subtitle line is true black text (fully invisible on black); Nexteer (red) and AIC (red/yellow) clear 4.5:1 with zero background and needed no change. A small light-plate compromise was tried for the two low-contrast logos and shipped first, but the owner asked for it removed too in a follow-up ("remove their light plate too", after being shown the legibility trade-off) — see the next addendum. `OrgLogo.tsx`'s grayscale-at-rest treatment is gone sitewide either way, including the Parcours timeline's inline logos, which previously used `grayscale opacity-70` unconditionally.
...
Verified after this pass: `npm run lint` (still 0 errors, 146 pre-existing warnings) and a clean `npm run build` (56/56 static pages) both pass; the GitHub links and the logo treatment were confirmed live in the browser on the homepage, `/parcours`, and `/contact`.

**Addendum — second follow-up, same day:** two more owner requests.
- **Hero photo supplied.** The owner-provided AI-generated (Gemini) portrait — transparent PNG, navy suit, arms crossed — is now `public/images/hero-cutout.png`, closing the "still open" item from the initial build. The source file (2400×1792) had a large transparent margin around the subject (bounding box only 888×1334 of the full canvas), which made the portrait render tiny inside `Hero.tsx`'s `object-contain` mount when used as-is; cropped to the alpha channel's bounding box (Pillow `getbbox()`) before saving so the subject fills the mount properly, matching the reference's prominent cutout. `hasHeroPhoto()`'s existence check now resolves true; the grid-pattern fallback no longer renders on the homepage.
- **Organisation-logo light plate removed entirely, on request.** After seeing the trade-off (Stellantis' navy wordmark and ENSET's black subtitle text are now genuinely hard to read on black), the owner chose to drop the light-plate compromise anyway and show all four logos identically: original color, zero background, everywhere. `OrganisationInfo.needsLightPlate` is removed from `lib/organisations.ts` (not left as an unused `false` flag), and both `OrgLogo.tsx` and the Parcours page's inline logo rendering are back to one unconditional treatment. This is a known, accepted legibility trade-off, not a bug — don't "fix" it again without asking first.

Verified again: `npm run lint` (0 errors, 146 pre-existing warnings) and `npm run build` (clean, 56/56 pages) both pass; the hero photo and delisted logo plate were confirmed live in the browser (Next.js's image-optimizer disk cache had to be cleared — `.next/cache/images` plus a full `.next` wipe — after replacing `hero-cutout.png` mid-session, since it caches by path, not content).
